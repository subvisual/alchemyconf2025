        ...
        <div className="relative">
          ...
          {schedule[`workshopsDay${day}` as keyof typeof schedule]
            .map((item: any, index: number) => (
              <div
                key={`${day}-${index}`}
                className={`relative flex gap-3 ${item.type === "date" && mobile ? "hidden" : "pb-8 tablet:gap-8 tablet:pb-10 desktop:gap-10 desktop:pb-12"}`}
              >
                {/* TIMELINE DOT */}
                <div className="mt-2 h-3 w-3 rounded-full border-2 border-bordeux bg-bordeux tablet:h-4 tablet:w-4 desktop:h-[18px] desktop:w-[18px]"></div>
